<template>
  <div class="starforce-clean-shell bg-[#0A0A0A] text-[#FBFAFC] min-h-screen font-inter selection:bg-[#5A1BD1]/30 overflow-hidden pb-10">
    ...
    <section class="pt-24 md:pt-28 pb-20 px-4 flex flex-col items-center text-center max-w-[1200px] mx-auto mt-6">
      <div :class="['inline-flex items-center gap-3.5 px-5 py-2.5 rounded-full border border-white/10 bg-white/5 text-xs md:text-sm font-normal text-[#FBFAFC] mb-8 reveal-init', { 'reveal-active': isPillRevealed }]">
        <span class="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_10px_#22c55e] animate-pulse"></span>
        Cinematic Color Made Simple
      </div>
      
      <h1 class="text-[40px] md:text-[110px] font-medium leading-[46px] md:leading-[121px] tracking-[-1.5px] md:tracking-[-5.5px] text-[#FBFAFC] mb-8 overflow-hidden flex flex-col items-center">
        <span :class="['reveal-line reveal-init whitespace-nowrap', { 'reveal-active': isH1Line1Revealed }]">Hollywood Color.</span>
        <span :class="['reveal-line font-instrument font-normal italic md:tracking-normal text-[#FBFAFC] pt-1 reveal-init', { 'reveal-active': isH1Line2Revealed }]">Straight From Your iPhone.</span>
      </h1>
      
      <p :class="['text-base md:text-[28px] font-medium md:leading-[42px] md:tracking-[-0.6px] text-[#999999] mb-12 max-w-4xl reveal-init', { 'reveal-active': isElementsRevealed }]">
      ...
      <div :class="['flex flex-col sm:flex-row gap-4 mb-20 w-full sm:w-auto px-4 justify-center reveal-init', { 'reveal-active': isElementsRevealed }]">
        <button @click.stop.prevent="shopifyCheckout" :disabled="isCheckoutLoading" class="bg-[#5A1BD1] text-[#FBFAFC] px-8 py-4 rounded-full font-medium hover:bg-[#6b2af0] transition shadow-[0_0_30px_rgba(90,27,209,0.4)] disabled:opacity-50 whitespace-nowrap w-auto inline-self-center mx-auto text-sm md:text-base">
          {{ isCheckoutLoading ? 'Opening Checkout...' : `Buy Now — ${selectedCurrency.symbol}${currentPrice} ${selectedCurrency.code}` }}
        </button>
      </div>

      <div v-reveal class="w-full rounded-[32px] md:rounded-[48px] bg-[#0D0D0D] p-1.5 md:p-2 border border-white/5 custom-inner-bevel relative overflow-hidden shadow-2xl">
        <div class="rounded-[26px] md:rounded-[40px] overflow-hidden aspect-video relative group z-0">
    ...
    <section id="versatility" class="px-2 md:px-4 max-w-[1200px] mx-auto mb-8">
      <div class="bg-[#0D0D0D] rounded-[32px] md:rounded-[48px] py-16 md:py-32 px-4 md:px-16 flex flex-col items-center text-center custom-inner-bevel relative overflow-hidden">
        ...
        <div class="relative z-20 flex flex-col items-center w-full">
          ...
          <h2 class="text-4xl md:text-[70px] font-medium leading-[44px] md:leading-[84px] md:tracking-[-3.5px] text-[#FBFAFC] mb-6 flex flex-col items-center overflow-hidden">
            <span v-reveal class="reveal-line">One LUT Pack. <br class="md:hidden" /> <span class="font-instrument font-normal italic md:tracking-normal text-[#FBFAFC]">Any Content.</span></span>
          </h2>
          
          <p v-reveal class="text-base md:text-[22px] font-medium md:leading-[33px] md:tracking-[-0.4px] text-[#999999] mb-12 max-w-3xl px-2">
            Move the slider to see the difference. Flat iPhone footage instantly becomes deep and cinematic.
          </p>

          <div 
            class="w-full p-1.5 md:p-2 rounded-2xl md:rounded-3xl bg-[#0A0A0A]/60 border border-white/10 shadow-2xl relative select-none mb-8" 
            @mousemove="handleDrag" 
            @touchmove="handleDrag" 
            ref="sliderContainer"
          >
            <div class="relative rounded-xl md:rounded-2xl overflow-hidden aspect-[16/10] z-10 bg-[#0A0A0A]">
              <div class="absolute inset-0 z-0">
                <nuxt-img :src="`/slides/${activeTag.filePrefix}_before.jpg`" sizes="sm:100vw md:100vw" format="webp" quality="80" loading="lazy" class="w-full h-full object-cover pointer-events-none" alt="Before LUT" />
                <span class="absolute bottom-4 left-4 md:bottom-6 md:left-6 bg-[#050505]/80 backdrop-blur text-[10px] md:text-xs px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-neutral-300 border border-white/10 font-mono font-bold">Apple Log</span>
              </div>
              
              <div class="absolute inset-0 z-10" :style="{ clipPath: `polygon(0 0, ${sliderPos}% 0, ${sliderPos}% 100%, 0 100%)` }">
                <nuxt-img :src="`/slides/${activeTag.filePrefix}_after.jpg`" sizes="sm:100vw md:100vw" format="webp" quality="80" loading="lazy" class="w-full h-full object-cover pointer-events-none" alt="After LUT" />
                <span class="absolute bottom-4 right-4 md:bottom-6 md:right-6 bg-[#5A1BD1]/80 backdrop-blur text-[10px] md:text-xs px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-white border border-white/20 font-mono font-bold">SF Kodak 500T</span>
              </div>
              ...
              <div class="absolute top-0 bottom-0 w-0 z-30 pointer-events-none" :style="{ left: `${sliderPos}%` }">
                ...
                <div class="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/30 shadow-xl pointer-events-auto cursor-ew-resize flex items-center justify-center bg-gradient-to-b from-white/20 to-transparent backdrop-blur-md transition-transform duration-200 active:scale-90 hover:scale-105">
                  <div class="flex gap-[3px] justify-center items-center">
                    <div class="w-[2px] h-2.5 md:h-3 bg-white/80 rounded-full"></div>
                    <div class="w-[2px] h-2.5 md:h-3 bg-white/80 rounded-full"></div>
                  </div>
                </div>
              </div>
          ...
          <div class="w-full overflow-x-auto scrollbar-hide border-b border-white/5 relative mb-4">
            <div class="flex items-center justify-center min-w-max md:min-w-0 pb-4 relative px-2 gap-4 md:gap-16 mx-auto" ref="tabsContainer">
              <button v-for="(tag, index) in useCases" :key="tag.label"
                :ref="el => { if (el) tabRefs[index] = el }"
                @click="setTab(tag, index)"
                class="text-xs md:text-xl font-bold tracking-tight transition-all duration-300 px-4 py-2.5 md:px-6 md:py-3.5 flex items-center gap-2 rounded-xl md:rounded-2xl focus:outline-none bg-transparent border-none outline-none cursor-pointer whitespace-nowrap shrink-0 transform active:scale-95"
                :class="activeTag.label === tag.label ? 'text-[#5A1BD1]' : 'text-neutral-400 hover:text-neutral-200'">
                <svg class="w-4 h-4 md:w-6 md:h-6 hidden sm:inline-block" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
    ...
    <section id="features" class="px-4 max-w-[1200px] mx-auto mb-16">
      <div class="bg-[#0D0D0D] rounded-[32px] md:rounded-[48px] py-20 md:py-32 px-4 md:px-16 flex flex-col items-center text-center custom-inner-bevel relative overflow-hidden">
        ...
        <div class="relative z-20 flex flex-col items-center w-full">
          ...
          <div class="grid md:grid-cols-2 gap-6 md:gap-8 w-full text-left mb-8">
            <div v-reveal class="bg-[#0A0A0A]/60 border border-white/5 rounded-[24px] md:rounded-3xl p-6 md:p-8 custom-inner-bevel flex flex-col">
              <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/5 flex items-center justify-center text-lg md:text-xl mb-6 border border-white/10">⚡</div>
              ...
              <div class="space-y-4 text-sm md:text-base font-medium leading-[24px] md:leading-[27px] text-[#999999]">
                <div>
            ...
            <div v-reveal class="bg-[#0A0A0A]/60 border border-white/5 rounded-[24px] md:rounded-3xl p-6 md:p-8 custom-inner-bevel flex flex-col justify-between">
              <div>
                <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/5 flex items-center justify-center text-lg md:text-xl mb-6 border border-white/10">📖</div>
                <h3 class="text-2xl md:text-[28px] font-medium md:leading-[28px] md:tracking-[-0.6px] text-[#FBFAFC] mb-4">PDF Installation Guide</h3>
                <p class="text-sm md:text-base font-medium text-[#FBFAFC] mb-2">Simple step-by-step instructions.</p>
                <div class="text-[#999999] text-sm md:text-base font-medium space-y-2 leading-[24px] md:leading-[27px]">
                  <p>A beginner-friendly guide showing you exactly how to install and use the pack directly on your mobile device.</p>
                  <ul class="text-xs md:text-sm text-neutral-500 space-y-2 pl-4 list-disc border-t border-white/5 pt-3 mt-3">
              ...
              <div class="mt-6 text-[10px] md:text-xs font-mono text-neutral-500 uppercase pt-4 border-t border-white/5">Starforce_Manual_v1.pdf</div>
            </div>
          </div>

          <div v-reveal class="w-full bg-[#0A0A0A]/60 border border-white/5 rounded-[24px] md:rounded-3xl p-6 md:p-8 custom-inner-bevel text-left">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              <div class="max-w-xl">
                ...
                <p class="text-[#999999] text-sm md:text-base font-medium leading-[24px] md:leading-[27px]">
              ...
              <div class="bg-white/5 border border-white/10 px-5 py-3 rounded-xl text-xs text-[#FBFAFC] font-mono shrink-0 font-bold opacity-50 w-full sm:w-auto text-center sm:text-left">
    ...
    <section id="compatibility" class="py-16 md:py-24 px-4 max-w-[1200px] mx-auto flex flex-col items-center text-center">
      <div v-reveal class="inline-flex items-center justify-center px-5 py-2 rounded-full border border-white/10 bg-white/5 text-xs text-[#999999] mb-8">
        Compatibility
      </div>
      <h2 class="text-4xl md:text-[70px] font-medium leading-[44px] md:leading-[84px] md:tracking-[-3.5px] text-[#FBFAFC] mb-6 flex flex-col items-center overflow-hidden">
        <span v-reveal class="reveal-line">Works With <br class="md:hidden" /> <span class="font-instrument font-normal italic md:tracking-normal text-[#FBFAFC]">Almost Every App</span></span>
      </h2>
      <p v-reveal class="text-base md:text-[22px] font-medium md:leading-[33px] md:tracking-[-0.4px] text-[#999999] mb-16 max-w-4xl">
      ...
      <div class="w-full flex flex-col overflow-hidden relative py-2">
        <div class="absolute left-0 top-0 bottom-0 w-16 md:w-24 bg-gradient-to-r from-[#0A0A0A] to-transparent z-20 pointer-events-none"></div>
        <div class="absolute right-0 top-0 bottom-0 w-16 md:w-24 bg-gradient-to-l from-[#0A0A0A] to-transparent z-20 pointer-events-none"></div>
        ...
        <div class="marquee-container flex gap-1.5 items-center">
          <div class="marquee-content flex gap-1.5 min-w-full justify-around animate-marquee-left">
            <div v-for="app in activeApps" :key="app.name" class="flex items-center bg-[#0D0D0D] border border-white/5 hover:border-white/10 transition px-5 py-3.5 md:px-7 md:py-4 rounded-xl md:rounded-2xl text-base md:text-[22px] font-medium text-[#FBFAFC] shrink-0 custom-inner-bevel">
              <span class="bg-clip-text text-transparent bg-gradient-to-r font-mono" :class="app.gradientClass">{{ app.name }}</span>
            </div>
          </div>
          <div class="marquee-content flex gap-1.5 min-w-full justify-around animate-marquee-left">
            <div v-for="app in activeApps" :key="app.name + '-loop'" class="flex items-center bg-[#0D0D0D] border border-white/5 hover:border-white/10 transition px-5 py-3.5 md:px-7 md:py-4 rounded-xl md:rounded-2xl text-base md:text-[22px] font-medium text-[#FBFAFC] shrink-0 custom-inner-bevel">
              <span class="bg-clip-text text-transparent bg-gradient-to-r font-mono" :class="app.gradientClass">{{ app.name }}</span>
            </div>
          </div>
        </div>
    ...
    <section id="reviews" class="px-2 md:px-4 max-w-[1200px] mx-auto mb-16">
      <div class="bg-[#0D0D0D] rounded-[32px] md:rounded-[48px] py-20 md:py-32 px-2 md:px-16 flex flex-col items-center text-center custom-inner-bevel relative overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-b from-transparent via-[#0A0A0A]/40 to-[#0A0A0A] z-10 pointer-events-none"></div>
        ...
        <div class="relative z-20 flex flex-col items-center w-full">
          ...
          <h2 class="text-4xl md:text-[70px] font-medium leading-[44px] md:leading-[84px] md:tracking-[-3.5px] text-[#FBFAFC] mb-6 flex flex-col items-center overflow-hidden">
            <span v-reveal class="reveal-line">What Creators <br class="md:hidden" /> <span class="font-instrument font-normal italic md:tracking-normal text-[#FBFAFC]">Are Saying</span></span>
          </h2>
          
          <p v-reveal class="text-base md:text-[22px] font-medium md:leading-[33px] md:tracking-[-0.4px] text-[#999999] mb-16 md:mb-24 max-w-3xl px-4">
            Real feedback from filmmakers, vloggers and content creators transforming mobile footage.
          </p>

          <div class="w-full md:hidden space-y-4 overflow-hidden relative py-2">
            <div class="marquee-container flex gap-4 items-center">
              <div class="marquee-content flex gap-4 min-w-full animate-marquee-left">
                <div v-for="(review, index) in reviews.slice(0, 3)" :key="'mob1-'+index" 
                     class="bg-[#0A0A0A]/60 border border-white/5 p-5 rounded-2xl custom-inner-bevel flex flex-col justify-between w-[280px] shrink-0 text-left">
                  <div>
                    <div class="flex gap-0.5 text-orange-400 text-[10px] mb-3"><span>★</span><span>★</span><span>★</span><span>★</span><span>★</span></div>
                    <p class="text-xs font-medium leading-[18px] text-[#FBFAFC]/90 mb-4 font-light italic">"{{ review.text }}"</p>
                  </div>
                  <div class="flex items-center gap-2 border-t border-white/5 pt-3">
                    <img :src="review.avatar" :alt="review.name" class="w-8 h-8 rounded-full object-cover border border-white/20" />
                    <div class="flex flex-col min-w-0">
                      <h4 class="text-xs font-bold text-[#FBFAFC] truncate">{{ review.name }}</h4>
                      <p class="text-[10px] text-neutral-500 font-mono truncate">{{ review.username }}</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="marquee-content flex gap-4 min-w-full animate-marquee-left">
                <div v-for="(review, index) in reviews.slice(0, 3)" :key="'mob1-loop-'+index" 
                     class="bg-[#0A0A0A]/60 border border-white/5 p-5 rounded-2xl custom-inner-bevel flex flex-col justify-between w-[280px] shrink-0 text-left">
                  <div>
                    <div class="flex gap-0.5 text-orange-400 text-[10px] mb-3"><span>★</span><span>★</span><span>★</span><span>★</span><span>★</span></div>
                    <p class="text-xs font-medium leading-[18px] text-[#FBFAFC]/90 mb-4 font-light italic">"{{ review.text }}"</p>
            ...
            <div class="marquee-container flex gap-4 items-center">
              <div class="marquee-content flex gap-4 min-w-full animate-marquee-right">
                <div v-for="(review, index) in reviews.slice(3, 6)" :key="'mob2-'+index" 
                     class="bg-[#0A0A0A]/60 border border-white/5 p-5 rounded-2xl custom-inner-bevel flex flex-col justify-between w-[280px] shrink-0 text-left">
                  <div>
                    <div class="flex gap-0.5 text-orange-400 text-[10px] mb-3"><span>★</span><span>★</span><span>★</span><span>★</span><span>★</span></div>
                    <p class="text-xs font-medium leading-[18px] text-[#FBFAFC]/90 mb-4 font-light italic">"{{ review.text }}"</p>
              ...
              <div class="marquee-content flex gap-4 min-w-full animate-marquee-right">
                <div v-for="(review, index) in reviews.slice(3, 6)" :key="'mob2-loop-'+index" 
                     class="bg-[#0A0A0A]/60 border border-white/5 p-5 rounded-2xl custom-inner-bevel flex flex-col justify-between w-[280px] shrink-0 text-left">
                  <div>
                    <div class="flex gap-0.5 text-orange-400 text-[10px] mb-3"><span>★</span><span>★</span><span>★</span><span>★</span><span>★</span></div>
                    <p class="text-xs font-medium leading-[18px] text-[#FBFAFC]/90 mb-4 font-light italic">"{{ review.text }}"</p>
          ...
          <div class="hidden md:grid grid-cols-3 gap-6 w-full text-left">
    ...
    <section id="faq" class="py-16 md:py-24 max-w-3xl mx-auto px-4">
      <div class="text-center mb-16">
        <div v-reveal class="inline-flex items-center justify-center px-5 py-2 rounded-full border border-white/10 bg-white/5 text-xs text-[#999999] mb-8">
          Questions
        </div>
        <h2 class="text-4xl md:text-5xl font-medium text-[#FBFAFC] tracking-[-1px] md:tracking-[-2px] flex flex-col items-center overflow-hidden">
      ...
      <div class="space-y-4 mb-16">
        <div v-reveal v-for="(item, index) in faqs" :key="index" class="rounded-2xl md:rounded-3xl bg-[#0D0D0D] overflow-hidden transition-all duration-300 custom-inner-bevel">
          <button @click="toggleFaq(index)" class="w-full px-5 py-5 md:px-6 md:py-6 text-left flex justify-between items-center font-medium text-base md:text-[18px] text-[#FBFAFC]">
            {{ item.q }}
            <span class="text-xl md:text-2xl font-light transition-transform duration-300 text-[#999999]" :class="{ 'rotate-180 text-[#5A1BD1]': openFaq === index }">↓</span>
          </button>
          <div v-show="openFaq === index" class="px-5 pb-6 md:px-6 md:pb-8 text-[#999999] text-sm md:text-[18px] leading-[22px] md:leading-[27px] font-medium border-t border-white/5 pt-4">
            {{ item.a }}
          </div>
        </div>
      </div>

      <div v-reveal class="w-full rounded-2xl md:rounded-3xl p-5 sm:p-8 bg-[#0D0D0D] border border-white/5 text-left space-y-4 custom-inner-bevel">
    ...
    <section class="py-16 md:py-24 px-4 flex flex-col items-center">
      <div v-reveal class="relative w-full max-w-md bg-[#0D0D0D] rounded-[32px] md:rounded-[40px] p-6 md:p-10 overflow-hidden custom-inner-bevel">
        ...
        <div class="relative z-20 flex flex-col">
          ...
          <div class="flex items-end gap-1 mb-8">
            <span class="text-4xl md:text-6xl font-medium text-[#FBFAFC]">{{ selectedCurrency.symbol }}{{ currentPrice }}</span>
            <span class="text-[#999999] mb-1 md:make-mb-2 font-medium text-xs md:text-base">/ {{ selectedCurrency.code }}</span>
          </div>
          <p class="text-sm md:text-[18px] text-[#999999] font-medium mb-8 md:mb-10 leading-[22px] md:leading-[27px] tracking-[-0.4px]">One-time payment. Lifetime usage license metrics.</p>
          
          <div class="flex flex-col gap-4 mb-10 items-center justify-center">
            <button @click.stop.prevent="shopifyCheckout" :disabled="isCheckoutLoading" class="bg-[#5A1BD1] text-[#FBFAFC] py-3.5 md:py-4 px-12 rounded-full text-sm md:text-base font-medium hover:bg-[#6b2af0] transition disabled:opacity-50 whitespace-nowrap w-auto inline-self-center mx-auto">
              {{ isCheckoutLoading ? 'PROCESSING...' : 'BUY NOW' }}
            </button>
          </div>

          <ul class="space-y-4 md:space-y-5 text-sm md:text-base text-[#999999] font-medium">
            <li v-for="item in pricingFeatures" :key="item.text" class="flex items-center gap-3">
              <span class="text-[#FBFAFC] text-base md:text-xl shrink-0">✦</span> 
              <span>{{ item.text }}</span>
              <span v-if="item.comingSoon" class="text-[9px] font-mono font-bold bg-[#5A1BD1]/20 text-[#5A1BD1] px-1.5 py-0.5 rounded ml-auto shrink-0 border border-[#5A1BD1]/30">COMING SOON</span>
            </li>
          </ul>
        </div>
      </div>
    ...
    <footer class="pt-16 pb-4 px-4 md:px-8 max-w-[1200px] mx-auto flex flex-col md:flex-row justify-between items-center text-xs md:text-sm font-medium text-[#999999] gap-4">
      ...
      <div class="whitespace-nowrap text-center md:text-right">
    ...
    <transition name="modal-fade">
      <div v-if="showModal" class="fixed inset-0 z-[200] flex items-center justify-center p-4" @mousedown.self="closeModal">
        ...
        <div class="relative w-full max-w-md bg-[#0D0D0D] border border-white/10 rounded-[28px] md:rounded-[32px] p-6 md:p-8 shadow-2xl z-10 transform custom-inner-bevel flex flex-col">
          ...
          <div v-if="!isSuccess">
            ...
            <h3 class="text-2xl md:text-3xl font-medium text-white tracking-tight mb-3">
              GET 10% OFF <br/>
              <span class="font-instrument font-normal italic text-[#FBFAFC]">YOUR PACK</span>
            </h3>
            <p class="text-neutral-400 text-xs md:text-sm mb-6 font-light leading-relaxed">
