<template>
  <div class="video-detail-page bg-black fixed inset-0 z-[10000] flex flex-col items-center justify-center">
    
    <NuxtLink to="/">
      <svg
        class="fixed left-1/2 z-[11000] -translate-x-1/2 logo-svg text-[#ffc200] cursor-pointer"
    ...
    <NuxtLink 
      to="/" 
      class="absolute top-8 left-8 text-white/50 hover:text-yellow-400 transition-colors z-[10010] uppercase tracking-widest text-[10px] flex items-center gap-2"
    >
      <span class="text-lg">✕</span> Close Project
    </NuxtLink>

    <button @click="navigate(-1)" class="nav-arrow left-4 md:left-10" aria-label="Previous video">
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M15 19l-7-7 7-7" />
      </svg>
    </button>

    <button @click="navigate(1)" class="nav-arrow right-4 md:right-10" aria-label="Next video">
      <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M9 5l7 7-7 7" />
      </svg>
    </button>

    <div class="flex flex-col items-center gap-6 video-container">
      <div class="relative w-[75vw] aspect-video bg-zinc-900 shadow-2xl overflow-hidden">
        <iframe
          :key="currentVideoId"
          :src="`https://player.vimeo.com/video/${currentVideoId}?autoplay=1&color=ffc200`"
      ...
      <div class="info-fade text-center w-full">
        <h1 class="project-title">
          {{ currentTitle }} by Gene Perez
        </h1>
      </div>
    </div>
  </div>
...
// Corrected Order to match Component IDs
const videoIds = ['1089378829', '1089402338', '1089411022', '982811608', '982811608']
...
const currentVideoId = computed(() => route.params.id)
const currentIndex = computed(() => videoIds.indexOf(currentVideoId.value))
const currentTitle = computed(() => titles[currentIndex.value] || 'Untitled Project')

function navigate(direction) {
  let newIndex = currentIndex.value + direction
  if (newIndex < 0) newIndex = videoIds.length - 1
  if (newIndex >= videoIds.length) newIndex = 0
  router.push(`/video/${videoIds[newIndex]}`)
}
...
function updateLogoSize() {
  ...
  if (vw <= 850) {
    ...
    document.documentElement.style.setProperty('--logo-height', '32px')
    document.documentElement.style.setProperty('--logo-top', '14px')
  } else {
    document.documentElement.style.setProperty('--logo-width', '170px')
    document.documentElement.style.setProperty('--logo-height', '52px')
    document.documentElement.style.setProperty('--logo-top', '14px')
  }
}
...
.project-title {
  ...
  line-height: 1;
  text-align: center;
  /* 0.7 scale of original caption size variable */
  font-size: calc(var(--section-caption-size, 2vw) * 0.7);
  text-transform: none;
  font-weight: 400;
